...
def createPriceTables(startDate, endDate, deltaInMinutes, varParameters, basicParameters, priceTableName):
    ...
    timePeriods_df = pd.DataFrame(timeArray, columns=['Timestamp', 'Daytype', 'VariableFee', 'Tag'])
    ...
    timePeriods_df = pd.merge(  left=baseTimePriceArray_df,
                                right=timePeriods_df,
                                how="left",
                                on=["Timestamp"],
                                validate="one_to_one")
...
def createCostTable(tarifConfigName, dataTableName, priceTableName, interval):
    ...
    energyCostInTime_df['energyCost'] = energyCostInTime_df['ActivePowerConsumption'] * energyCostInTime_df['VariableFee'] * varParameters['diffVarPriceMultiplyer']
