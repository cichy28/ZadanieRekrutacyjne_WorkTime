...
import { ICommand, ICommandBaseDocument } from "@models/commands";
...
export async function getUserCommnads(userId: string) {
	console.log("asdas PIZDA");
	let test = await commandModel.findLastCommandFromUser("JCJC");
	console.log(test);
...
// export async function prepareChartData(rawData: ICommandBaseDocument[]) {
